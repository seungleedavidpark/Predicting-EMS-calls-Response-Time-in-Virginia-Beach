...
mapTheme <- theme(text = element_text(family = "Avenir", color = "black", face = "bold"),
...
mainems.sf <- main_ems.sf %>%
  mutate(Date_time_of_day = str_c)
...
census_api_key("cf27cad00b4f276277286253707f61fc1b3e3831", overwrite = TRUE)
...
ggplot() + 
  ...
  geom_sf(data = call_duration_net %>%
...
ggplot() + 
  geom_sf(data = vb_boundary, fill = "black") +
  geom_sf(data = monday_aftrenoon_net %>%
            dplyr::filter(Call_to_OnScene > 0), aes(fill = Call_to_OnScene)) +
  scale_fill_viridis_c(option="plasma") +
  geom_sf(data = ems_stations, color="white") +
  labs(title= "Mean call duration by fishnet") +
  mapTheme

##functionalize mapping daily timeofday reponsetime
map_daily_timeofday_responsetime <-
      function(main_ems.sf, dotw = "Mon", time_of_day = "Afternoon") {
        dat <- main_ems.sf %>% 
                filter(`dotw` == dotw & `time_of_day` == time_of_day) %>%
                mutate(Call_to_OnScene = as.numeric(Call_to_OnScene)) %>% 
                aggregate(., vb_fishnet, mean) %>%
                mutate(Call_to_OnScene = replace_na(Call_to_OnScene, 0),
                    uniqueID = rownames(.),
...
map_daily_timeofday_responsetime(main_ems.sf, "Fri","Morning")
...
ggplot() +
  geom_sf(data = vb_boundary, fill = "black") +
  geom_sf(data = vb_ems_fishnet %>%
            dplyr::filter(countEMS > 0), aes(fill = countEMS), color = NA) +
  scale_fill_viridis(option="plasma") +
  labs(title = "Fishnet of EMS calls in Virginia Beach")

### Map call duration by fishnet
call_duration_net <- 
  dplyr::select(main_ems.sf) %>% 
  mutate(Call_to_OnScene = as.numeric(main_ems.sf$Call_to_OnScene)) %>% 
  aggregate(., vb_fishnet, mean) %>%
...
PopDens_net <- 
  dplyr::select(vb_census) %>% 
  mutate(PopDens = as.numeric(vb_census$PopDens)) %>% 
  aggregate(., vb_fishnet, mean) %>%
...
ggplot(dat, aes(x=PopDens, y=Call_to_OnScene)) +
  geom_point() +
  plotTheme
...
ggplot() + 
  geom_sf(data = vb_boundary, fill = "black") +
  geom_sf(data = MedAge_net%>%
            dplyr::filter(Med_Age > 0), color = NA, aes(fill = Med_Age)) +
  scale_fill_viridis_c(option="brewer yellow-green-blue") +
  labs(title= "Median Age across Virginia Beach") +
  mapTheme

ggplot(dat, aes(x=Med_Age, y=Call_to_OnScene)) +
  geom_point() +
  plotTheme
