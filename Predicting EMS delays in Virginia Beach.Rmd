...
author: "Al-Jalil Gault & David Seunglee Park"
date: "12/15/2020"
output: 
  html_document:
    code_folding: hide
...
nn_function <- function(measureFrom,measureTo,k) {
  ...
  output <-
    ...
    group_by(thisPoint) %>%
    dplyr::summarise(pointDistance = mean(point_distance)) %>%
    arrange(as.numeric(thisPoint)) %>% 
...
# load color palettes
palette6 <- c("#F4EDCA", "#FFDB6D", "#C4961A","#D16103", "#800000")
...
Also the final maps shows the response time error on the out of fold sets. It shows that the errors
...
ggplot() +
  geom_sf(data = vb_boundary, fill = "black") +
  geom_sf(data = map_preds, aes(colour = q5(ResponseTime.AbsError),na.rm=TRUE),
          show.legend = "point", size = 0.00005) +
