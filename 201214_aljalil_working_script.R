...
preds.train <- data.frame(pred   = predict(reg_final_train, newdata = vb_training),
...
for (i in seq_along(voronoi_polygon)) {
  voronoi_polygon[[i]]@data = cbind(voronoi_polygon[[i]]@data, unique.id = i)
}
